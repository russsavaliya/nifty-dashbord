...
function RSIArc({ value }: { value: number }) {
  ...
  const bgColor = value > 70 ? '#fef2f2' : value < 30 ? '#f0fdf4' : '#fefce8';
...
function StatCard({ icon, label, value, sub, color }: { icon: string; label: string; value: string; sub?: string; color: 'green' | 'red' | 'yellow' | 'blue' }) {
  ...
  return (
    <div className={`rounded-xl p-4 border ${colors.border} ${colors.bg} flex flex-col gap-2`}>
      ...
      {sub && <span className="text-slate-400 text-xs font-mono">{sub}</span>}
    </div>
  );
}
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 fade-in shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-slate-900 font-semibold text-sm">Technical Indicators</h3>
      ...
      <div className="flex items-center justify-center mb-5 py-2 bg-slate-50 rounded-xl border border-slate-200">
        <div className="flex flex-col items-center">
          <span className="text-slate-400 text-xs mb-2 uppercase tracking-wider">RSI (14)</span>
          <RSIArc value={rsi} />
        </div>
      </div>
